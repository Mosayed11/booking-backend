...
class Resource(Base):
    ...
    resource_metadata = sa.Column(sa.JSON, server_default='{}')
    created_at = sa.Column(sa.DateTime(timezone=True), server_default=func.now())
...
class Booking(Base):
    ...
    resource_id = sa.Column(UUID(as_uuid=True), sa.ForeignKey('resources.id', ondelete='CASCADE'), nullable=False)
    start_at = sa.Column(sa.DateTime(timezone=True), nullable=False)
    end_at = sa.Column(sa.DateTime(timezone=True), nullable=False)
    status = sa.Column(ENUM(BookingStatus, name='booking_status'), server_default='pending')
